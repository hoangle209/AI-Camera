...
from paddle.nn import Conv2D, Dropout, LayerList, LayerNorm, Linear
...
zeros_ = constant_(value=0.0)
ones_ = constant_(value=1.0)
...
class Transformer(nn.Layer):
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(1, 25):
            ...
            if paddle.equal_all(
                preds_idx, paddle.full(paddle.shape(preds_idx), 3, dtype="int64")
            ):
                break
    ...
    def forward_beam(self, images):
        ...
        def beam_decode_step(
            ...
            def predict_word(
                dec_seq, enc_output, n_active_inst, n_bm, memory_key_padding_mask
            ):
...
class TransformerEncoderLayer(nn.Layer):
    ...
    def __init__(
        ...
        self.conv1 = Conv2D(
            in_channels=d_model, out_channels=dim_feedforward, kernel_size=(1, 1)
        )
        self.conv2 = Conv2D(
            in_channels=dim_feedforward, out_channels=d_model, kernel_size=(1, 1)
        )
