...
class MultiheadAttention(nn.Layer):
    ...
    def forward(
        ...
        q *= self.scaling
        q = paddle.transpose(
            paddle.reshape(q, [q_shape[0], q_shape[1], self.num_heads, self.head_dim]),
            [1, 2, 0, 3],
        )
        k = paddle.transpose(
            paddle.reshape(
        ...
        attn_output_weights = paddle.matmul(q, paddle.transpose(k, [0, 1, 3, 2]))
        ...
        if key_padding_mask is not None:
            ...
            y = paddle.full(shape=paddle.shape(key), dtype="float32", fill_value="-inf")
            y = paddle.where(key == 0.0, key, y)
            attn_output_weights += y
        attn_output_weights = F.softmax(
            attn_output_weights.astype("float32"),
            axis=-1,
            dtype=paddle.float32
            if attn_output_weights.dtype == paddle.float16
