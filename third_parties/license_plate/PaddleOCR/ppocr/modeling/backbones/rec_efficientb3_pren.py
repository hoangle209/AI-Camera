...
class ConvBlock(nn.Layer):
    def __init__(self, block_params):
        ...
        output_filters = self.block_args.input_filters * self.block_args.expand_ratio
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if (
            ...
            and self.input_filters == self.final_oup
        ):
        ...
        for i, block_params in enumerate(self.blocks_params):
            block_params = block_params._replace(
                input_filters=EffUtils.round_filters(
                    block_params.input_filters, self.global_params
                ),
                output_filters=EffUtils.round_filters(
                    block_params.output_filters, self.global_params
                ),
                num_repeat=EffUtils.round_repeats(
                    block_params.num_repeat, self.global_params
                ),
            )
            self.blocks.append(self.add_sublayer(f"{i}-0", ConvBlock(block_params)))
            concerned_idx += 1
            ...
            for j in range(block_params.num_repeat - 1):
                self.blocks.append(
                    self.add_sublayer(f"{i}-{j + 1}", ConvBlock(block_params))
                )
                concerned_idx += 1
                if concerned_idx in self.concerned_block_idxes:
                    self.out_channels.append(block_params.output_filters)
