...
def load_model(config, model, optimizer=None, model_type="det"):
    ...
    if model_type == "vqa":
        ...
        if checkpoints:
            ...
            logger.info(f"resume from {checkpoints}")
...
def load_pretrained_params(model, path):
    ...
    assert os.path.exists(path + ".pdparams"), f"The {path}.pdparams does not exists!"
    ...
    for k1 in params.keys():
        if k1 not in state_dict.keys():
            logger.warning(f"The pretrained params {k1} not in model")
        else:
...
def save_model(
    ...
    **kwargs,
    ...
    if is_best:
        logger.info(f"save best model is to {model_prefix}")
    else:
        logger.info(f"save model in {model_prefix}")
