...
from ppocr.utils.e2e_metric.polygon_fast import area, area_of_intersection, iod
...
def get_socre_A(gt_dir, pred_dict):
    ...
    def gt_reading_mod(gt_dict):
        ...
        for i in range(n):
            ...
            if text != "":
                xx[4] = np.array([text], dtype=f"U{len(text)}")
...
def get_socre_B(gt_dir, img_id, pred_dict):
    ...
    def gt_reading_mod(gt_dir, gt_id):
        gt = io.loadmat("{}/poly_gt_img{}.mat".format(gt_dir, gt_id))
        gt = gt["polygt"]
        return gt
