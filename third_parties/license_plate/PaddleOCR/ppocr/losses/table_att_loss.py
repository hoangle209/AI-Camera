...
class TableAttentionLoss(nn.Layer):
    def __init__(
        self, structure_weight, loc_weight, use_giou=False, giou_weight=1.0, **kwargs
    ):
        super().__init__()
    ...
    def giou_loss(self, preds, bbox, eps=1e-7, reduction="mean"):
        ...
        iw = paddle.clip(ix2 - ix1 + 1e-3, 0.0, 1e10)
        ih = paddle.clip(iy2 - iy1 + 1e-3, 0.0, 1e10)
        ...
        # union
        uni = (
            (preds[:, 2] - preds[:, 0] + 1e-3) * (preds[:, 3] - preds[:, 1] + 1e-3)
            + (bbox[:, 2] - bbox[:, 0] + 1e-3) * (bbox[:, 3] - bbox[:, 1] + 1e-3)
            - inters
            + eps
        )
        ...
        ew = paddle.clip(ex2 - ex1 + 1e-3, 0.0, 1e10)
        eh = paddle.clip(ey2 - ey1 + 1e-3, 0.0, 1e10)
    ...
    def forward(self, predicts, batch):
        ...
        if self.use_giou:
            loc_loss_giou = (
                self.giou_loss(loc_preds * loc_targets_mask, loc_targets)
                * self.giou_weight
            )
