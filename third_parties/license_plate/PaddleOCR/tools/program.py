    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                cur_metric_str = "cur metric, {}".format(
                    ", ".join([f"{k}: {v}" for k, v in cur_metric.items()])
                )
                ...
                best_str = "best metric, {}".format(
                    ", ".join([f"{k}: {v}" for k, v in best_model_dict.items()])
                )
                ...
                if log_writer is not None:
                    log_writer.log_metrics(
                        metrics={
                            f"best_{main_indicator}": best_model_dict[main_indicator]
                        },
                        prefix="EVAL",
                        step=global_step,
                    )
        ...
        if dist.get_rank() == 0 and epoch > 0 and epoch % save_epoch_step == 0:
            save_model(
                ...
                config,
                is_best=False,
                prefix=f"iter_epoch_{epoch}",
                best_model_dict=best_model_dict,
                epoch=epoch,
                global_step=global_step,
            )
            if log_writer is not None:
                log_writer.log_model(is_best=False, prefix=f"iter_epoch_{epoch}")
...
def preprocess(is_train=False):
    ...
    if is_train:
        ...
        log_file = f"{save_model_dir}/train.log"
    else:
        log_file = None
    ...
    if use_xpu:
        device = "xpu:{}".format(os.getenv("FLAGS_selected_xpus", 0))
    else:
        device = f"gpu:{dist.ParallelEnv().dev_id}" if use_gpu else "cpu"
    ...
    if "use_visualdl" in config["Global"] and config["Global"]["use_visualdl"]:
        save_model_dir = config["Global"]["save_model_dir"]
        vdl_writer_path = f"{save_model_dir}/vdl/"
    ...
    if (
        ...
        wandb_writer_path = f"{save_dir}/wandb"
        if "wandb" in config:
    ...
    logger.info(f"train with paddle {paddle.__version__} and device {device}")
