...
def draw_kie_result(batch, node, idx_to_cls, count):
    ...
    logger.info(f"The Kie Image saved in {save_path}")
...
def main():
    ...
    with open(save_res_path, "w") as fout:
        ...
        fout.close()
    logger.info("success!")
    logger.info(f"It took {np.sum(count_t)} s for predict {len(count_t)} images.")
    ips = len(count_t[warmup_times:]) / np.sum(count_t[warmup_times:])
    logger.info(f"The ips is {ips} images/s")
