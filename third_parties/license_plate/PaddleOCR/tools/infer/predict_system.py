...
class TextSystem:
    ...
    def __call__(self, img, cls=True):
        ...
        logger.debug(f"dt_boxes num : {len(dt_boxes)}, elapse : {elapse}")
        if dt_boxes is None:
            return None, None
        img_crop_list = []
        ...
        if self.use_angle_cls and cls:
            ...
            logger.debug(f"cls num  : {len(img_crop_list)}, elapse : {elapse}")

        rec_res, elapse = self.text_recognizer(img_crop_list)
        logger.debug(f"rec_res num  : {len(rec_res)}, elapse : {elapse}")
...
def main(args):
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        print(
            "rec_res : ", rec_res
        )  # [('59-P1', 0.9204786419868469), ('664.80', 0.9899836182594299)]
        total_time += elapse

        logger.debug(
            str(idx) + "  Predict time of {}: {:.3f}s".format(image_file, elapse)
        )
    ...
    logger.info(f"The predict total time is {time.time() - _st}")
    if args.benchmark:
    ...
    if args.total_process_num > 1:
        save_results_path = os.path.join(
            draw_img_save_dir, f"system_results_{args.process_id}.txt"
        )
    else:
    ...
    if args.use_mp:
        ...
        for process_id in range(total_process_num):
            cmd = (
                [sys.executable, "-u"]
                + sys.argv
                + [f"--process_id={process_id}", f"--use_mp={False}"]
            )
