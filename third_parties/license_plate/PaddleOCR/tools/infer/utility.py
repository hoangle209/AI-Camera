...
def init_args():
    ...
    parser.add_argument("--gpu_mem", type=int, default=200)

    # params for text detector
    parser.add_argument("--image_dir", type=str, default="./img-test/test_1")
    parser.add_argument("--det_algorithm", type=str, default="DB")
    parser.add_argument("--det_model_dir", type=str, default="./en_PP-OCRv3_det_infer")
...
def create_predictor(args, mode, logger):
    ...
    if model_dir is None:
        logger.info(f"not find {mode} model file path {model_dir}")
        sys.exit(0)
    if args.use_onnx:
        ...
        if not os.path.exists(params_file_path):
            raise ValueError(f"not find params file path {params_file_path}")
        ...
        if args.use_gpu:
            gpu_id = 0  # get_infer_gpuid()
            print("paddel load device : ", gpu_id)
            Debug_log(
                currentframe(),
            ...
            config.enable_use_gpu(args.gpu_mem, gpu_id)
