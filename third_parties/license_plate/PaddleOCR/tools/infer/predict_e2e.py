...
class TextE2E:
    def __init__(self, args):
        ...
        if self.e2e_algorithm == "PGNet":
            ...
            logger.info(f"unknown e2e_algorithm:{self.e2e_algorithm}")
            sys.exit(0)
...
if __name__ == "__main__":
    ...
    for image_file in image_file_list:
        ...
        if count > 0:
            total_time += elapse
        count += 1
        logger.info(f"Predict time of {image_file}: {elapse}")
        src_im = utility.draw_e2e_res(points, strs, image_file)
        img_name_pure = os.path.split(image_file)[-1]
        img_path = os.path.join(draw_img_save, f"e2e_res_{img_name_pure}")
        cv2.imwrite(img_path, src_im)
        logger.info(f"The visualized image saved in {img_path}")
    if count > 1:
        logger.info(f"Avg Time: {total_time / (count - 1)}")
