...
from tools.infer.utility import draw_boxes, draw_ocr, str2bool
        ...
        for region in res:
            if region["type"] == "Table":
                ...
                for text_result in region["res"]:
                    f.write(f"{json.dumps(text_result)}\n")
...
def main(args):
    ...
    for image_file in image_file_list:
        ...
        total_time += elapse
        logger.info("Predict time of {}: {:.3f}s".format(image_file, elapse))
        ...
        if args.visualize:
            ...
            if "structure_table" in args.server_url:
                to_excel(res["html"], f"./{img_name}.xlsx")
        ...
        cnt += 1
        if cnt % 100 == 0:
            logger.info(f"{cnt} processed")
    logger.info(f"avg time cost: {float(total_time) / cnt}")
